...
from .models import Product, Vulnerability
from .Kafka.producer import produce_cpe_message
import requests
...
def product_list(request):
    ...
    cpe_names = [product.cpe_name for product in page_obj if product.cpe_name]
...
def vulnerability_product(request, product_cpe_name):
    ...
    cpes = Product.objects.filter(cpe_name=cpe_name)

    for cpe in cpes:
        vulnerabilities = Vulnerability.objects.filter(product=cpe)
        return render(request, 'products/vulnerability_product.html', {'cve_s': vulnerabilities, 'cpe': cpe})
